...
**Project layout:** in Lightroom export, use **Put in Subfolder** so files land in `{project-slug}/exports` inside the category folder, for example:

`/Volumes/T9/05_EXPORTS/WEB_FULL/architecture/220-hudson/exports/`

Set **`ENABLE_PROJECT_EXPORT_PIPELINE=1`** in `tools/.env` so the watcher debounces, runs `tools/process-export-project.mjs` (social WebPs, `metadata/`, `reports/studio-os-report-payload.json`), then performs the usual Google Sheet + R2 upload for each file in `exports/`. If that variable is unset, exports in `exports/` still upload to R2 + the sheet (no debounce or local derivatives from the watcher). Optional `project.json` at the project folder root can set `project`, `client`, `location`, and `category` for the Studio handoff JSON (used when you run the post-processor).
