...
export default function ClientGalleryView({ token }: { token: string }) {
  ...
  const toggleSelection = useCallback(
    ...
    [gallery?.showSelectionTools, token]
  );
    ...
    async (imageId: string) => {
      ...
      try {
        const res = await fetch("/api/client/download", {
          ...
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({ token, imageId, type: "single" }),
        });
        const data = (await res.json()) as {
          ok: boolean;
          downloadUrl?: string;
  ...
  const downloadFavorites = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/client/download", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ token, type: "favorites" }),
      });
      const data = (await res.json()) as {
        ok: boolean;
  ...
  if (status === "error") {
    ...
    return (
      ...
      >
        <p
          className={`text-sm ${expired ? "text-amber-900" : forbidden ? "text-black/80" : "text-red-200"}`}
          role="alert"
          aria-live="polite"
        >
          {error}
        ...
        <p
          className={`mt-3 text-xs ${
            expired ? "text-amber-800/90" : forbidden ? "text-black/50" : "text-red-200/70"
          }`}
        >
          {expired
