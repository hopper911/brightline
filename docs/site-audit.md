...
- `app/work/[section]/page.tsx` — Pillar listing (architecture, advertising, corporate)
...
- **Public work:** `lib/portfolioPillars.ts` — 3 pillars: `architecture`, `advertising`, `corporate` (maps to Prisma `WorkSection`: REA/TRI→architecture, ACD/CUL→advertising, BIZ→corporate). Nav is built from `PILLARS`.
...
- Pillar cards from `PILLARS` — `/work/{architecture|advertising|corporate}`
- Positioning / approach / structured delivery / AI blocks — `lib/config/strategicPositioning.ts`
