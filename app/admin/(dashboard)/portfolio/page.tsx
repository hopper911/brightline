...
export default function AdminPortfolioPage() {
  ...
  /** Pending gallery rows from Browse R2 (saved on submit). */
  const [portfolioR2Gallery, setPortfolioR2Gallery] = useState<
  ...
  async function onR2GalleryKeys(picks: R2BrowserPick[]) {
    ...
    const resolved = await Promise.all(
      keys.map(async (raw) => {
        ...
        const url = await resolvePublicUrl(storageKey);
        return { storageKey, url };
      })
    );
    setPortfolioR2Gallery((prev) => [...prev, ...resolved]);
  }
  ...
  async function uploadFile(file: File, projectSlug: string) {
    ...
    const uploadHeaders =
      "headers" in data && data.headers && typeof data.headers === "object"
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const newImages = [
        ...
        ...portfolioR2Gallery.map((row, index) => ({
          url: row.url,
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-16">
      ...
      <p className="mt-2 max-w-2xl text-xs text-black/55">
        Portfolio = visual feed and taxonomy here (PortfolioProject). Public pillar case studies live on
        Work — not the same data model.
      </p>
      {/* TODO: auto-feed / tighter sync with Work + Studio CMS when product is ready */}
      <div className="mt-4">
        ...
        <div className="mt-4 grid gap-6">
          ...
          <section className="grid gap-4 rounded-xl border border-black/10 bg-black/[0.02] p-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                type="button"
                className="btn btn-ghost mt-6 shrink-0 self-end text-sm"
                onClick={() => setR2CoverOpen(true)}
              >
                Browse R2
              </button>
            </div>
          ...
          <section className="grid gap-4 rounded-xl border border-black/10 bg-black/[0.02] p-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {portfolioR2Gallery.length > 0 ? (
                <span className="text-xs text-black/50">
                  {portfolioR2Gallery.length} image(s) from R2 will be added on save
                </span>
              ) : null}
            </div>
            {portfolioR2Gallery.length > 0 ? (
              <ul className="space-y-2 rounded-lg border border-black/10 bg-white p-3 text-sm">
                {portfolioR2Gallery.map((row, index) => (
                  <li
                    key={`${row.storageKey}-${index}`}
                    className="flex items-center justify-between gap-2"
                  >
                    <span className="min-w-0 truncate font-mono text-xs text-black/70">
                      {row.storageKey}
                    </span>
                    <button
                      type="button"
                      className="btn btn-ghost shrink-0 text-xs"
                      onClick={() =>
                        setPortfolioR2Gallery((prev) => prev.filter((_, i) => i !== index))
                      }
                    >
                      Remove
                    </button>
