...
export async function POST(req: Request) {
  ...
  if (await isRateLimitedAsync(getClientIp(req), { scope: "r2-tools", max: 40, windowMs: 60 * 60_000 })) {
    return NextResponse.json({ ok: false, error: "Too many R2 tool requests." }, { status: 429 });
  }

  let body: { op?: string; prefix?: string; maxKeys?: number; vault?: string };
  try {
  ...
  try {
    ...
    const prefix = normalizePrefix(assertR2ManagerKeyAllowed(rawPrefix, vault));
    const maxKeys = Math.min(typeof body.maxKeys === "number" ? body.maxKeys : 2000, 5000);
    const flat = await listFlatWithSizes(prefix, maxKeys, vault);
    ...
    if (op === "duplicates") {
      const groups = groupDuplicateKeys(flat.objects).map((g) => ({
        ...
        count: g.keys.length,
        keys: g.keys,
        items: g.keys.map((key, i) => mapListed({ key, size: g.sizes[i] ?? 0 }, vault)),
      }));
      return NextResponse.json({
        ok: true,
        prefix,
