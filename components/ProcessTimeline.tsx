...
const STEPS = [
  ...
  { title: "Shoot", desc: "On-location capture with clear shot list." },
  { title: "Proofs", desc: `Initial selects in ${CREDIBILITY?.turnaround?.proofs ?? "5–7 days"}.` },
  { title: "Finals", desc: `Delivery in ${CREDIBILITY?.turnaround?.finals ?? "10–14 days"}.` },
];
      ...
      {STEPS.map((step, i) => (
        <li key={step.title} className="flex gap-4">
          ...
          <div>
            ...
            <p className={`mt-0.5 text-sm ${isDark ? "text-white/60" : "text-black/60"}`}>{step.desc}</p>
          </div>
        </li>
      ))}
    </ol>
