...
import { createPublishingBrightlineWorkProjectHandler } from "@/lib/platform/jobs/handlers/publishing-brightline-work-project";
...
export function registerDefaultJobHandlers(
  ...
  if (!registry.has(PUBLISHING_BRIGHTLINE_WORK_PROJECT_JOB)) {
    registry.register(
      PUBLISHING_BRIGHTLINE_WORK_PROJECT_JOB,
      createPublishingBrightlineWorkProjectHandler(undefined, provider)
    );
  }
}
